...
const Toolbar = ({ selected, clearSelected, todos, setTodos }) => {
  ...
  const addTodo = () => {
    if (newTodo) {
      setTodos([
        ...todos,
        { id: Math.random(), todo: newTodo, completed: false },
      ]);
      setNewTodo("");
    }
  };
  ...
  return (
    <div
      style={{ width: "40%", display: "flex", flexDirection: "row-reverse" }}
    >
      {selected.length ? (
        <Button onClick={deleteTodos} style={{ color: "red" }}>
          -
        </Button>
      ) : null}
      {selected.length ? (
        <Button onClick={completeTodos} style={{ color: "green" }}>
          ✓
        </Button>
      ) : null}
      <div style={{ display: "flex" }}>
        <input value={newTodo} onChange={(e) => setNewTodo(e.target.value)} />
        <Button onClick={addTodo} style={{ color: "green" }}>
          +
        </Button>
      </div>
    </div>
...
export const Todo = () => {
  const [todos, setTodos] = useState([
    ...
    { id: 2, todo: "code", completed: false },
  ...
  return (
    <div style={{ width: "80%", position: "relative" }}>
      <Template
        ...
        toolbar={useCallback(
          ({ ...props }) => (
            <Toolbar todos={todos} setTodos={setTodos} {...props} />
          ),
          [todos]
        )}
        rowHeight={40}
